...
class ConfigurationValidationError(ValidationError):
    ...
    def __init__(
        ...
        context = {"config_field": config_field, "config_source": config_source}
        ...
        super().__init__(message=message, context=context, suggestions=suggestions)
...
class NetworkConnectionError(NetworkError):
    ...
    def __init__(
        ...
        context = {"host": host, "port": port, "service_name": service_name}
...
class ServiceAlreadyExistsError(UserError):
    ...
    def __init__(self, service_name: str, config_source: str | None = None):
        message = f"Service '{service_name}' already exists in configuration"

        context = {"service_name": service_name, "config_source": config_source}
        ...
        technical_details = (
            f"Service name '{service_name}' conflicts with existing configuration"
        )
    ...
    def __init__(
        ...
        if available_services:
            ...
            similar = [
                ...
                if s.lower().startswith(service_name.lower()[:2])
            ]
            if similar:
                suggestions.insert(
                    1, f"Did you mean one of these? {', '.join(similar[:3])}"
                )
...
class KubernetesResourceNotFoundError(UserError):
    ...
    def __init__(
        ...
        similar_resources: list[str] | None = None,
    ):
        if namespace:
        ...
        if namespace:
            suggestions.insert(
                2, f"Check the namespace: kubectl get {resource_type}s -n {namespace}"
            )
        else:
            suggestions.insert(
                2, "Try specifying a different namespace with --namespace flag"
            )

        if similar_resources:
            suggestions.insert(
                1, f"Did you mean one of these? {', '.join(similar_resources[:3])}"
            )

        technical_details = (
            f"Resource {resource_type}/{resource_name} not found via kubectl"
        )
...
class MultipleNamespacesFoundError(UserError):
    ...
    def __init__(
        self, resource_name: str, namespaces: list[str], resource_type: str = "service"
    ):
        message = (
            f"Kubernetes {resource_type} '{resource_name}' found in multiple namespaces"
        )
        ...
        suggestions = [
            ...
            "Or use the interactive prompt to select a namespace",
        ]
...
class NoPortsAvailableError(UserError):
    ...
    def __init__(
        self, resource_name: str, namespace: str, resource_type: str = "service"
    ):
        ...
        if resource_type == "service":
            ...
            suggestions.insert(
                1, "Check if the deployment pods have containerPort specifications"
            )

        technical_details = (
            f"No port specifications found in {resource_type} {resource_name}"
        )
