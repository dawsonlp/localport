...
class KubernetesResource:
    ...
    def __post_init__(self) -> None:
        ...
        if self.resource_type not in valid_types:
            raise ValueError(
                f"Resource type must be one of {valid_types}, got {self.resource_type}"
            )
    ...
    def _validate_kubernetes_name(name: str, field_name: str) -> None:
        ...
        if not re.match(r"^[a-z0-9.-]+$", name):
            raise ValueError(
                f"Kubernetes {field_name} must contain only lowercase letters, numbers, hyphens, and dots"
            )
        ...
        if (
            ...
            raise ValueError(
                f"Kubernetes {field_name} cannot start or end with hyphen or dot"
            )
    ...
    def from_kubectl_output(
        cls, name: str, namespace: str, resource_type: str, kubectl_json: dict[str, Any]
    ) -> "KubernetesResource":
        ...
        if resource_type == "service":
            ...
            for port_spec in ports_spec:
                ...
                if port_num:
                    ...
                    actual_port = (
                        target_port if isinstance(target_port, int) else port_num
                    )
                    discovered_port = DiscoveredPort(
                        port=actual_port, protocol=protocol, name=port_name
                    )
            ...
            for container in containers:
                ...
                for port_spec in container_ports:
                    ...
                    if port_num:
                        discovered_port = DiscoveredPort(
                            port=port_num, protocol=protocol, name=port_name
                        )
                        available_ports.append(discovered_port)
