...
class ConnectionInfo:
    ...
    def _validate_kubectl_config(self) -> None:
        ...
        for field in required_fields:
            if field not in self.config:
                raise ValueError(
                    f"kubectl connection missing required field '{field}'. Example: resource_name: 'my-service'"
                )
        ...
        if "namespace" in self.config and not self.config["namespace"].strip():
            raise ValueError(
                "kubectl namespace cannot be empty if provided. Use a valid namespace like 'default' or 'production', or remove the field"
            )
        ...
        if "resource_type" in self.config:
            ...
            if self.config["resource_type"] not in valid_types:
                raise ValueError(
                    f"kubectl resource_type '{self.config['resource_type']}' is invalid. Valid options: {', '.join(valid_types)}"
                )
    ...
    def _validate_ssh_config(self) -> None:
        ...
        for field in required_fields:
            if field not in self.config:
                raise ValueError(
                    f"SSH connection missing required field '{field}'. Example: host: 'example.com' or host: '192.168.1.100'"
                )
        ...
        if "port" in self.config:
            try:
                ...
                if not 1 <= port <= 65535:
                    raise ValueError(
                        f"SSH port {port} is invalid. Use a port between 1 and 65535 (default SSH port is 22)"
                    )
        ...
        if "remote_host" in self.config and self.config["remote_host"]:
            if not self.config["remote_host"].strip():
                raise ValueError(
                    "SSH remote_host cannot be empty if provided. Use a hostname like 'example.com' or remove the field"
                )
    ...
    def validate_against_discovered_resource(self, discovered_resource) -> list[str]:
        ...
        if self.technology != ForwardingTechnology.KUBECTL:
            raise ValueError(
                "Can only validate kubectl connections against discovered resources"
            )
        ...
        if not isinstance(discovered_resource, KubernetesResource):
            raise ValueError(
                "discovered_resource must be a KubernetesResource instance"
            )
        ...
        if self.get_kubectl_resource_name() != discovered_resource.name:
            warnings.append(
                f"Resource name mismatch: expected '{discovered_resource.name}', got '{self.get_kubectl_resource_name()}'"
            )

        # Validate namespace matches
        if self.get_kubectl_namespace() != discovered_resource.namespace:
            warnings.append(
                f"Namespace mismatch: expected '{discovered_resource.namespace}', got '{self.get_kubectl_namespace()}'"
            )

        # Validate resource type matches
        if self.get_kubectl_resource_type() != discovered_resource.resource_type:
            warnings.append(
                f"Resource type mismatch: expected '{discovered_resource.resource_type}', got '{self.get_kubectl_resource_type()}'"
            )

        # Check if resource has any ports
        if not discovered_resource.has_ports:
            warnings.append(
                f"Resource '{discovered_resource.name}' has no exposed ports"
            )
    ...
    def update_from_discovered_resource(self, discovered_resource) -> "ConnectionInfo":
        ...
        if self.technology != ForwardingTechnology.KUBECTL:
            raise ValueError(
                "Can only update kubectl connections from discovered resources"
            )
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        """
        return {"technology": self.technology.value, "config": dict(self.config)}
