...
class Port:
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(self.value, int):
            raise ValueError(
                f"Port must be an integer, got {type(self.value).__name__}. Example: port: 8080 or port: 5432"
            )

        if not self.is_valid_port(self.value):
            raise ValueError(
                f"Port {self.value} is invalid. Use a port between 1 and 65535 (common ports: 80, 443, 8080, 5432)"
            )
