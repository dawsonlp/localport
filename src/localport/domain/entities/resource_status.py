...
class ResourceStatus:
    ...
    node_name: str | None = None  # For pods
    pod_ip: str | None = None  # For pods
    host_ip: str | None = None  # For pods
    ...
    def is_healthy(self) -> bool:
        ...
        if self.resource_type == ResourceType.POD:
            return (
                self.phase == ResourcePhase.RUNNING
                and self.ready
                and self.restart_count < 5  # Arbitrary threshold
            )
        elif self.resource_type == ResourceType.NODE:
            # Check for Ready condition
            ready_condition = next(
                (c for c in self.conditions if c.type == "Ready"), None
            )
    ...
    def has_condition(self, condition_type: str, status: str = "True") -> bool:
        ...
        """
        return any(
            c.type == condition_type and c.status == status for c in self.conditions
        )

    def get_condition(self, condition_type: str) -> ResourceCondition | None:
        """
        ...
        """
        return next((c for c in self.conditions if c.type == condition_type), None)
