...
class HealthMonitorScheduler:
    ...
    async def start_monitoring(self, services: list[Service]) -> None:
        ...
        logger.info(
            "Health monitor scheduler received services for monitoring",
        ...
        for service in services:
            ...
            if has_health_check:
                service_info["health_check_type"] = service.health_check_config.get(
                    "type"
                )
                service_info["health_check_interval"] = service.health_check_config.get(
                    "interval", 30
                )
                service_info["failure_threshold"] = service.health_check_config.get(
                    "failure_threshold", 3
                )
            ...
            if will_monitor:
                ...
                if not has_health_check:
                    logger.info(
                        "Health monitor skipping service - no health check config",
                        **service_info,
                    )
                elif not is_running:
                    logger.info(
                        "Health monitor skipping service - not running", **service_info
                    )
                else:
                    logger.warning(
                        "Health monitor skipping service - unknown reason",
                        **service_info,
                    )

        logger.info(
            "Health monitor service analysis complete",
            total_services=len(services),
        ...
        logger.info(
            "Health monitoring startup complete",
    ...
    async def stop_monitoring(self, service_ids: set[UUID] | None = None) -> None:
        ...
        if service_ids is None:
            ...
            logger.info(
                "Stopping health monitoring for services",
    ...
    async def _start_service_monitoring(self, service: Service) -> None:
        ...
        cooperative_task = ServiceHealthMonitorTask(
            service=service, health_scheduler=self, check_interval=check_interval
        )
        ...
        logger.info(
            "Started cooperative health monitoring for service",
            service_name=service.name,
    ...
    async def add_service(self, service: Service) -> None:
        ...
        if (
            ...
            and service.status == ServiceStatus.RUNNING
            and self._running
        ):
            ...
            logger.info("Added service to health monitoring", service_name=service.name)
    ...
    async def remove_service(self, service_id: UUID) -> None:
        ...
        logger.info(
            "Removed service from health monitoring", service_id=str(service_id)
        )
    ...
    async def _perform_health_check(self, service: Service) -> HealthCheckResult:
        ...
        try:
            ...
            if (
                cluster_aware
                and self._cluster_health_provider
                and service.technology == ForwardingTechnology.KUBECTL
            ):
                ...
                if cluster_context:
                    try:
                        logger.debug(
                            "Checking cluster health before service health check",
                            service_name=service.name,
                            cluster_context=cluster_context,
                        )

                        cluster_healthy = (
                            await self._cluster_health_provider.is_cluster_healthy(
                                cluster_context
                            )
                        )
                        ...
                        if not cluster_healthy:
                            ...
                            cluster_health = (
                                await self._cluster_health_provider.get_cluster_health(
                                    cluster_context
                                )
                            )
                            ...
                            if cluster_health and cluster_health.error_message:
                                ...
                                cluster_error = (
                                    f"Cluster unhealthy: {cluster_health.status.value}"
                                )

                            logger.warning(
                                "Skipping service health check due to cluster issues",
                                service_name=service.name,
                                cluster_context=cluster_context,
                            ...
                            logger.debug(
                                "Cluster is healthy, proceeding with service health check",
                                service_name=service.name,
                                cluster_context=cluster_context,
                            )
                        ...
                        logger.warning(
                            "Failed to check cluster health, proceeding with service check",
                            service_name=service.name,
                            cluster_context=cluster_context,
            ...
            health_checker = self._health_check_factory.create_health_checker(
                check_type, health_config.get("config", {})
            )
            ...
            if check_type == "tcp":
                ...
                if "url" not in check_config:
                    check_config["url"] = (
                        f"http://localhost:{service.local_port}/health"
                    )
            elif check_type == "kafka":
                if "bootstrap_servers" not in check_config:
                    check_config["bootstrap_servers"] = (
                        f"localhost:{service.local_port}"
                    )
    ...
    async def _trigger_service_restart(
        self, service: Service, health_result: HealthCheckResult
    ) -> None:
        ...
        if health_result.skip_restart_due_to_cluster:
            logger.info(
                "Skipping service restart due to cluster health issues",
                service_name=service.name,
        ...
        logger.critical(
            "Service restart triggered by health check failures",
            service_name=service.name,
            failure_count=self._failure_counts.get(service.id, 0),
        ...
        if restart_scheduled:
            ...
            logger.error(
                "Failed to schedule service restart",
                service_name=service.name,
...
class ServiceHealthMonitorTask(CooperativeTask):
    ...
    def __init__(
        ...
        """
        super().__init__(
            name=f"health_monitor_{service.name}", check_interval=check_interval
        )
    ...
    async def _execute_iteration(self) -> None:
        ...
        if not self._health_scheduler._running:
            logger.debug(
                "Health monitoring stopped, ending task",
        ...
        if self._service.id not in self._health_scheduler._cooperative_tasks:
            logger.debug(
                "Service no longer monitored, ending task",
                service_name=self._service.name,
            )
            await self.request_shutdown()
            return
        ...
        try:
            ...
            health_result = await self._health_scheduler._perform_health_check(
                self._service
            )
            ...
            if health_result.is_healthy:
                ...
                if self._health_scheduler._failure_counts.get(self._service.id, 0) > 0:
                    logger.info(
                        ...
                        service_name=self._service.name,
                        previous_failures=self._health_scheduler._failure_counts[
                            self._service.id
                        ],
                    )
                ...
                if failure_count >= failure_threshold:
                    logger.error(
                        "Service health failure threshold reached",
                        service_name=self._service.name,
                        failure_count=failure_count,
                    ...
                    await self._health_scheduler._trigger_service_restart(
                        self._service, health_result
                    )

        except Exception as e:
            logger.exception(
                "Error in health check iteration",
                service_name=self._service.name,
                error=str(e),
            )
    ...
    async def _handle_iteration_error(self, error: Exception) -> bool:
        ...
        logger.warning(
            "Health check iteration error, continuing",
            service_name=self._service.name,
            error=str(error),
        )
