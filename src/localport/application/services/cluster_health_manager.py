...
class ClusterHealthManager(ClusterHealthProvider):
    ...
    def __init__(self, default_config: ClusterMonitorConfig | None = None):
        """
    ...
    async def register_context(
        self, context: str, config: ClusterMonitorConfig | None = None
    ) -> None:
        """
    ...
    async def get_cluster_info(self, context: str) -> ClusterInfo | None:
    ...
    async def get_cluster_health(self, context: str) -> ClusterHealth | None:
    ...
    async def get_resource_status(
        self, context: str, namespace: str, resource_name: str, resource_type: str
    ) -> ResourceStatus | None:
    ...
    async def refresh_cluster_health(self, context: str) -> ClusterHealth | None:
    ...
    async def get_all_cluster_health(self) -> dict[str, ClusterHealth | None]:
        """
    ...
    async def get_unhealthy_clusters(self) -> list[str]:
        ...
        for context, monitor in self._monitors.items():
            try:
                ...
                unhealthy_contexts.append(
                    context
                )  # Consider failed checks as unhealthy
    ...
    async def refresh_all_clusters(self) -> dict[str, ClusterHealth | None]:
        ...
        # Collect results
        health_states = {}
        for context, result in zip(refresh_tasks.keys(), results, strict=False):
            if isinstance(result, Exception):
                logger.error(
                    f"Failed to refresh health for context {context}: {result}"
                )
                health_states[context] = None
            else:
