...
class ClusterConfigManager:
    ...
    def __init__(self):
        ...
        self._default_config: ClusterMonitorConfig | None = None
    ...
    def load_from_yaml_config(self, yaml_config: dict[str, Any]) -> None:
        ...
        self._default_config = self._create_cluster_monitor_config(
            cluster_health_config
        )
        ...
        for context, context_config in cluster_contexts.items():
            ...
            if context_cluster_health:
                # Merge with defaults
                merged_config = self._merge_cluster_configs(
                    cluster_health_config, context_cluster_health
                )
                self._context_configs[context] = self._create_cluster_monitor_config(
                    merged_config
                )

        logger.info(
            "Loaded cluster health configuration",
            extra={
                "default_enabled": (
                    self._default_config.enable_cluster_info
    ...
    def extract_cluster_contexts_from_services(
        ...
        for service in services:
            ...
            if service.technology == ForwardingTechnology.KUBECTL:
                try:
                    ...
                    logger.warning(
                        "Failed to extract context from service",
        ...
        logger.info(
            ...
            extra={
                ...
                "kubectl_services": len(
                    [
                        s
                        for s in services
                        if s.technology == ForwardingTechnology.KUBECTL
                    ]
                ),
            },
        )
    ...
    def _create_cluster_monitor_config(
        ...
        enable_events_on_failure = self._get_command_enabled(
            commands, "events_on_failure", True
        )
    ...
    def _get_command_enabled(
        self, commands_config: dict[str, Any], command_name: str, default: bool
    ) -> bool:
        """
    ...
    def validate_configuration(self) -> list[str]:
        ...
        default_errors = self._validate_cluster_monitor_config(
        ...
        for context, config in self._context_configs.items():
            context_errors = self._validate_cluster_monitor_config(
    ...
    def _validate_cluster_monitor_config(
        self, config: ClusterMonitorConfig, prefix: str
    ) -> list[str]:
        """
        ...
        if not any(
            ...
            errors.append(
                f"{prefix}: at least one command should be enabled for effective monitoring"
            )
