...
class ConnectionValidationService:
    ...
    async def validate_service_name(
        self, name: str, exclude_existing: bool = True
    ) -> list[str]:
        ...
        if not re.match(r"^[a-zA-Z0-9._-]+$", name):
            errors.append(
                "Service name can only contain letters, numbers, dots, hyphens, and underscores"
            )

        # Cannot start or end with special characters
        if name.startswith((".", "-", "_")) or name.endswith((".", "-", "_")):
            errors.append(
                "Service name cannot start or end with dots, hyphens, or underscores"
            )
        ...
        if exclude_existing and not errors:  # Only check if name is otherwise valid
            try:
                if await self.config_repository.service_exists(name):
                    ...
                    errors.append(
                        f"Service '{name}' already exists. Try: {', '.join(suggestions[:2])}"
                    )
            except Exception as e:
                logger.warning(
                    "Error checking service existence", name=name, error=str(e)
                )
    ...
    async def validate_port_availability(self, local_port: int) -> list[str]:
        ...
        if local_port in well_known_ports:
            ...
            errors.append(
                f"Warning: Port {local_port} is typically used by {service}. Consider using a different port"
            )
        ...
        try:
            ...
            try:
                ...
                if e.errno == 48:  # Address already in use
                    errors.append(
                        f"Port {local_port} is already in use by another process"
                    )
                elif e.errno == 13:  # Permission denied
                    if local_port < 1024:
                        errors.append(
                            f"Port {local_port} requires root privileges. Try a port >= 1024"
                        )
                    else:
            ...
            logger.warning(
                "Error checking port availability", port=local_port, error=str(e)
            )
    ...
    async def validate_ssh_host(
        self, host: str, port: int = 22, timeout: float = 5.0
    ) -> list[str]:
        ...
        try:
            ...
            try:
                ...
                if e.errno == 61:  # Connection refused
                    errors.append(
                        f"Connection refused to {host}:{port}. Check if SSH service is running"
                    )
                elif e.errno == 8:  # Name resolution failure
                    errors.append(
                        f"Cannot resolve hostname '{host}'. Check if the hostname is correct"
                    )
                else:
            ...
            logger.warning(
                "Error testing SSH host connectivity",
                host=host,
                port=port,
                error=str(e),
            )
    ...
    async def validate_port_range(
        self, port: int, port_type: str = "port"
    ) -> list[str]:
        ...
        if not (1 <= port <= 65535):
            errors.append(
                f"{port_type.capitalize()} {port} must be between 1 and 65535"
            )
    ...
    async def validate_kubernetes_resource_name(self, resource_name: str) -> list[str]:
        ...
        if not re.match(r"^[a-z0-9.-]+$", resource_name):
            errors.append(
                "Kubernetes resource name must contain only lowercase letters, numbers, hyphens, and dots"
            )

        # Cannot start or end with hyphen or dot
        if (
            resource_name.startswith("-")
            or resource_name.startswith(".")
            or resource_name.endswith("-")
            or resource_name.endswith(".")
        ):
            errors.append(
                "Kubernetes resource name cannot start or end with hyphen or dot"
            )
    ...
    async def validate_kubernetes_namespace(self, namespace: str) -> list[str]:
        ...
        if namespace and namespace in ["kube-system", "kube-public", "kube-node-lease"]:
            errors.append(
                f"Warning: '{namespace}' is a system namespace. Make sure this is intended"
            )
    ...
    async def validate_connection(
        ...
        if name:
            ...
            result.configuration_errors.append(
                "Service configuration missing 'name' field"
            )
        ...
        if technology == "ssh":
            ...
            if not host:
                result.ssh_connectivity_errors.append(
                    "SSH connection missing 'host' field"
                )
            else:
                ...
                if check_ssh_connectivity:
                    ...
                    if ssh_adapter is not None:
                        try:
                            adapter_errors = await ssh_adapter.validate_connection_info(
                                connection_info
                            )
                            result.ssh_connectivity_errors.extend(adapter_errors)
                        except Exception as e:
                            result.ssh_connectivity_errors.append(
                                f"SSH validation error: {e}"
                            )
                    else:
        ...
        if technology == "kubectl":
            ...
            if not resource_name:
                ...
                if (
                    not fmt_errors
                    and check_k8s_resource
                    and kubectl_adapter is not None
                ):
                    try:
