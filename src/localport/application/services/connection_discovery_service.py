...
class ConnectionDiscoveryService:
    ...
    async def discover_kubectl_connection(
        ...
        """
        logger.debug(
            "Discovering Kubernetes resource",
    ...
    async def _find_in_specific_namespace(
        self, resource_name: str, namespace: str, resource_type: str
    ) -> DiscoveryResult:
    ...
    async def _intelligent_namespace_discovery(
        ...
        try:
            ...
            if resource:
                logger.debug(
                    "Found resource in current namespace", namespace=current_namespace
                )
        ...
        try:
            ...
            if current_namespace != "default":
                ...
                resource = await self.discovery_repository.find_resource(
                    name=resource_name, namespace="default", resource_type=resource_type
                )

                if resource:
                    logger.debug("Found resource in default namespace")
                    return DiscoveryResult.found_single(
                        resource, warnings=warnings + ["Found in default namespace"]
                    )
        ...
        try:
            ...
            if not all_matches:
                ...
                return DiscoveryResult.found_single(
                    resource, warnings=warnings + [f"Found in namespace: {namespace}"]
                )
                ...
                logger.debug(
                    "Found resource in multiple namespaces", namespaces=namespaces
                )
    ...
    async def get_resource_suggestions(
        ...
        try:
            ...
            logger.warning(
                "Failed to get resource suggestions",
