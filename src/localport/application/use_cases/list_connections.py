...
class ListConnectionsUseCase:
        ...
        try:
            ...
            for service_config in services_config:
                try:
                    ...
                    logger.warning(
                        "Skipping invalid service configuration",
    ...
    def _apply_filters(
        self, services: list[ServiceSummary], request: ListConnectionsRequest
    ) -> list[ServiceSummary]:
        ...
        if request.filter_technology:
            filtered = [
                s for s in filtered if s.technology == request.filter_technology
            ]

        # Filter by tags
        if request.filter_tags:
            filtered = [
                s for s in filtered if any(tag in s.tags for tag in request.filter_tags)
            ]
    ...
    def _apply_sorting(
        self, services: list[ServiceSummary], sort_by: str
    ) -> list[ServiceSummary]:
