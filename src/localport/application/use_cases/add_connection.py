...
class AddConnectionUseCase:
    ...
    async def execute(self, request: AddConnectionRequest) -> AddConnectionResponse:
        ...
        try:
            ...
            return AddConnectionResponse.failure_response(
                service_name=request.service_name or "unknown", message=str(e)
            )
        except Exception as e:
            logger.exception(
                "Failed to add connection", service_name=request.service_name
            )
            return AddConnectionResponse.failure_response(
                service_name=request.service_name or "unknown",
    ...
    async def _handle_kubectl_connection(
        self, request: AddConnectionRequest
    ) -> AddConnectionResponse:
        ...
        if not discovery_result.selected_port and discovery_result.resource:
            if discovery_result.resource.has_ports:
                ...
                if discovery_result.has_single_port:
                    discovery_result.selected_port = (
                        discovery_result.resource.single_port
                    )
                else:
                    # Use default port selection logic
                    discovery_result.selected_port = (
                        discovery_result.resource.get_default_port()
                    )
            else:
                raise NoPortsAvailableError(
                    resource_name=request.connection_params["resource_name"],
                    namespace=discovery_result.resource.namespace,
                    resource_type=request.connection_params.get(
                        "resource_type", "service"
                    ),
                )

        # Create service configuration
        service_config = await self._create_kubectl_service_config(
        ...
        discovered_info = {
            ...
            "available_ports": [
                port.to_dict() for port in discovery_result.resource.available_ports
            ],
            "selected_port": (
                discovery_result.selected_port.to_dict()
                if discovery_result.selected_port
                else None
            ),
        }
    ...
    async def _handle_ssh_connection(
        ...
        # Create service configuration
        service_config = await self._create_ssh_service_config(
            request, local_port, remote_port
        )
    ...
    async def _discover_kubernetes_resource(
        self, request: AddConnectionRequest
    ) -> DiscoveryResult:
        ...
        try:
            if namespace:
                ...
                resource = await self.discovery_repository.find_resource(
                    name=resource_name, namespace=namespace, resource_type=resource_type
                )
                ...
                current_namespace = (
                    await self.discovery_repository.get_current_namespace()
                )
                resource = await self.discovery_repository.find_resource(
                    name=resource_name,
            ...
            logger.exception(
                "Error during resource discovery", resource_name=resource_name
            )
            return DiscoveryResult.not_found(warnings=[f"Discovery failed: {str(e)}"])

    async def _create_kubectl_service_config(
        self, request: AddConnectionRequest, discovery: DiscoveryResult
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "connection": {
                ...
                **{
                    k: v
                    for k, v in request.connection_params.items()
                    if k not in ["resource_name", "namespace", "resource_type"]
                    and v is not None
                },
            },
            **{
    ...
    async def _create_ssh_service_config(
        self, request: AddConnectionRequest, local_port: int, remote_port: int
    ) -> dict[str, Any]:
        ...
        return {
            ...
            "connection": {
                **request.connection_params,
                ...
                "remote_host": request.connection_params.get(
