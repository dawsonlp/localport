...
from ..dto.connection_dto import RemoveConnectionRequest, RemoveConnectionResponse
...
class RemoveConnectionUseCase:
    ...
    async def execute(
        self, request: RemoveConnectionRequest
        ...
        try:
            ...
            removed = await self.config_repository.remove_service_config(
            ...
            logger.error(
                "Service not found", service_name=request.service_name, error=str(e)
            )
            return RemoveConnectionResponse.failure_response(
                service_name=request.service_name, message=str(e)
            )
        except Exception as e:
            logger.exception(
                "Failed to remove connection", service_name=request.service_name
            )
            return RemoveConnectionResponse.failure_response(
                service_name=request.service_name,
    ...
    async def _is_service_running(self, service_name: str) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Could not check service running status",
