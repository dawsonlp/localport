    ...
    def from_path(cls, path: Path, is_active: bool = False) -> "ConfigFile":
        ...
        if config_file.exists:
            try:
                ...
                logger.warning(
                    "Failed to get file metadata", path=str(path), error=str(e)
                )
...
class ConfigPathManager:
    ...
    @staticmethod
    def find_active_config(config_path: str | None = None) -> ConfigFile | None:
    ...
    @staticmethod
    def get_all_config_files(config_path: str | None = None) -> list[ConfigFile]:
    ...
    async def get_service_count(config_file: ConfigFile) -> int:
        ...
        try:
            ...
            logger.warning(
                "Failed to count services in config file",
    ...
    @staticmethod
    async def format_config_status(
        config_path: str | None = None, show_all_paths: bool = True
    ) -> str:
        ...
        for config_file in config_files:
            if config_file.exists and config_file.readable:
                config_file.service_count = await ConfigPathManager.get_service_count(
                    config_file
                )
    ...
    @staticmethod
    async def format_search_paths_with_status(config_path: str | None = None) -> str:
