...
class ServiceLogManager:
    ...
    def _ensure_directories(self) -> None:
        ...
        try:
            ...
            logger.error(
                "failed_to_create_log_directories",
                error=str(e),
    ...
    def generate_service_id(self, service_name: str) -> str:
        ...
        hash_input = f"{service_name}_{timestamp}".encode()
            ...
            try:
                ...
                with open(log_file, "w", encoding="utf-8") as f:
                    ...
                    self._write_metadata_header(
                        f, service_name, service_id, service_config
                    )
    ...
    def _write_metadata_header(
        self, file_handle, service_name: str, service_id: str, service_config: dict
    ) -> None:
...
"""
    ...
    def get_log_file_path(self, service_id: str) -> Path | None:
    ...
    def rotate_log(self, service_id: str) -> Path | None:
        """
        ...
        with self._lock:
            ...
            try:
                ...
                logger.error(
                    "failed_to_rotate_service_log", service_id=service_id, error=str(e)
                )
                return None
            ...
            for log_file in self._service_dir.glob("*.log*"):
                try:
                    ...
                    if file_mtime < cutoff_time:
                        log_file.unlink()
                        logger.info(
                            "service_log_cleaned_up",
                            log_file=str(log_file),
                            age_days=(datetime.now() - file_mtime).days,
                        )
                except OSError as e:
                    logger.warning(
                        "failed_to_cleanup_log_file",
                        log_file=str(log_file),
                        error=str(e),
                    )
        except OSError as e:
        ...
        try:
            for log_file in self._service_dir.glob("*.log"):
                try:
                    ...
                    # Count rotated files
                    rotated_count = len(
                        list(self._service_dir.glob(f"{service_id}.log.*"))
                    )
    ...
    def remove_service_log(self, service_id: str) -> None:
        ...
        with self._lock:
            if service_id in self._active_logs:
                ...
                logger.info("service_log_removed", service_id=service_id)
