...
class KafkaHealthCheck(HealthChecker):
    ...
    def _check_kafka_sync(self, bootstrap_servers: str, timeout: float) -> bool:
        ...
        try:
            ...
            logger.debug(
                "Kafka health check failed - no brokers available",
                bootstrap_servers=bootstrap_servers,
            )
            return False
        except KafkaError as e:
            logger.debug(
                "Kafka health check failed - Kafka error",
                bootstrap_servers=bootstrap_servers,
                error=str(e),
            )
            return False
            ...
            logger.debug(
                "Kafka health check failed - unexpected error",
                bootstrap_servers=bootstrap_servers,
                error=str(e),
            )
            return False
    ...
    def _check_topic_sync(
        self, bootstrap_servers: str, topic_name: str, timeout: float = 10.0
    ) -> bool:
        ...
        try:
            ...
            logger.debug("Kafka topic check failed", topic=topic_name, error=str(e))
            return False
    ...
    async def check_producer_connectivity(self, config: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.debug(
                "Kafka producer check failed",
                bootstrap_servers=bootstrap_servers,
                error=str(e),
            )
            return False

    def _check_producer_sync(
        self, bootstrap_servers: str, timeout: float = 10.0
    ) -> bool:
        ...
        try:
            ...
            try:
                ...
                logger.debug(
                    "Kafka producer check passed", bootstrap_servers=bootstrap_servers
                )
                return True
    ...
    def validate_config(self, config: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not isinstance(bootstrap_servers, str) or not bootstrap_servers.strip():
                logger.error(
                    "Kafka health check invalid bootstrap_servers",
                    bootstrap_servers=bootstrap_servers,
                )
                return False
    ...
    def get_default_config(self) -> dict[str, Any]:
        ...
        """
        return {"bootstrap_servers": "localhost:9092", "timeout": 10.0}
