...
class HTTPHealthCheck(HealthChecker):
    ...
    async def check_health(self, config: dict[str, Any]) -> HealthCheckResult:
        ...
        expected_status_codes = merged_config.get(
    ...
    async def _perform_http_check(
        ...
        try:
            connector = aiohttp.TCPConnector(ssl=verify_ssl, limit=1, limit_per_host=1)
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.request(method, url) as response:
                    ...
                    if response.status not in expected_status_codes:
                        logger.debug(
                            "HTTP health check failed - unexpected status code",
                            url=url,
                    ...
                    if expected_content:
                        try:
                            ...
                            if expected_content not in content:
                                logger.debug(
                                    "HTTP health check failed - expected content not found",
                                    url=url,
                                    expected_content=expected_content,
                                )
                                return HealthCheckResult.unhealthy(
                            ...
                            logger.debug(
                                "HTTP health check failed - error reading content",
                                url=url,
                                error=str(e),
                            )
                            return HealthCheckResult.unhealthy(
            ...
            logger.debug("HTTP health check failed - timeout", url=url, timeout=timeout)
            return HealthCheckResult.unhealthy(
            ...
            logger.debug(
                "HTTP health check failed - client error", url=url, error=str(e)
            )
            return HealthCheckResult.unhealthy(
                message=f"HTTP {method} {url} client error", error=str(e)
            )
        except Exception as e:
            logger.error(
    ...
    async def check(self, url: str, timeout: float = 5.0, **kwargs) -> bool:
        ...
        try:
            connector = aiohttp.TCPConnector(
                ssl=self.verify_ssl, limit=1, limit_per_host=1
            )
            ...
            async with aiohttp.ClientSession(
                ...
                async with session.request(self.method, url) as response:
                    ...
                    logger.debug(
                        "HTTP health check passed", url=url, status_code=response.status
                    )
                    return True
    ...
    def validate_config(self, config: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not isinstance(expected_codes, list) or not all(
                isinstance(code, int) and 100 <= code <= 599 for code in expected_codes
            ):
                logger.error(
                    "HTTP health check invalid expected_status_codes",
