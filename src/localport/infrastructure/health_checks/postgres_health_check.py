...
class PostgreSQLHealthCheck(HealthChecker):
        ...
        try:
            ...
            try:
                async with psycopg.AsyncConnection.connect(
                    connection_string, connect_timeout=timeout
                ) as conn:
                ...
                return HealthCheckResult.unhealthy(
                    message=f"PostgreSQL database error: {str(e)}", error=str(e)
                )
    ...
    async def check(self, config: dict[str, Any]) -> bool:
        ...
        try:
            ...
            try:
                async with psycopg.AsyncConnection.connect(
                    connection_string, connect_timeout=self.timeout
                ) as conn:
                    # Execute a simple query to verify the connection works
                    async with conn.cursor() as cur:
                        await cur.execute("SELECT 1")
                        ...
                        if result and result[0] == 1:
                            ...
                            logger.debug(
                                "PostgreSQL health check failed - unexpected query result"
                            )
                            return False
    ...
    async def check_database_exists(
        self, config: dict[str, Any], database_name: str
    ) -> bool:
        ...
        try:
            ...
            async with psycopg.AsyncConnection.connect(
                ...
                async with conn.cursor() as cur:
                    ...
                    logger.debug(
                        "PostgreSQL database existence check",
    ...
    async def check_table_exists(
        self, config: dict[str, Any], table_name: str, schema: str = "public"
    ) -> bool:
        ...
        try:
            ...
            logger.debug(
                "PostgreSQL table existence check failed",
                table=f"{schema}.{table_name}",
                error=str(e),
            )
            return False
    ...
    async def check_user_permissions(
        self, config: dict[str, Any], required_permissions: list = None
    ) -> bool:
        ...
        try:
            ...
            async with psycopg.AsyncConnection.connect(
                ...
                async with conn.cursor() as cur:
                    ...
                    if not has_connect:
                        logger.debug(
                            "PostgreSQL user lacks CONNECT permission",
                            user=user,
                            database=database,
                        )
                        return False
                    ...
                    for permission in required_permissions:
                        ...
                        if permission.upper() in [
                            ...
                            if not (result and result[0]):
                                logger.debug(
                                    "PostgreSQL user lacks schema permissions",
                                    user=user,
                    ...
                    logger.debug(
                        "PostgreSQL user permissions check passed",
                        user=user,
            ...
            logger.debug(
                "PostgreSQL user permissions check failed",
                user=config.get("user", "postgres"),
                error=str(e),
            )
            return False
    ...
    async def get_server_version(self, config: dict[str, Any]) -> str | None:
        ...
        try:
            ...
            async with psycopg.AsyncConnection.connect(
                ...
                async with conn.cursor() as cur:
                    ...
                    if result:
                        version = result[0]
                        logger.debug(
                            "PostgreSQL server version retrieved", version=version
                        )
                        return version
            ...
            logger.debug("PostgreSQL server version check failed", error=str(e))
            return None
    ...
    async def check_connection_pool(
        ...
        try:
            ...
            logger.debug(
                "PostgreSQL connection pool test failed",
                pool_size=pool_size,
                error=str(e),
            )
            return False
    ...
    def validate_config(self, config: dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not isinstance(database, str) or not database.strip():
                logger.error(
                    "PostgreSQL health check invalid database", database=database
                )
                return False
            ...
            logger.error(
                "Error validating PostgreSQL health check config", error=str(e)
            )
            return False
