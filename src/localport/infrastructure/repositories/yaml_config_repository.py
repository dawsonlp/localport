...
class YamlConfigRepository(ConfigRepository):
    ...
    async def load_configuration(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.error(
                "Failed to load configuration", path=str(self.config_path), error=str(e)
            )
            raise
        ...
        try:
            ...
            if self.config_path.exists():
                try:
                    ...
                    logger.warning(
                        "Failed to create backup before save", error=str(backup_err)
                    )
            ...
            logger.error(
                "Failed to save configuration", path=str(self.config_path), error=str(e)
            )
            raise
    ...
    def _validate_service(self, service: dict[str, Any], index: int) -> list[str]:
        ...
        if technology is not None:
            ...
            if technology not in valid_technologies:
                errors.append(
                    f"{prefix}: technology must be one of {valid_technologies}"
                )
        ...
        for port_field in ["local_port", "remote_port"]:
            ...
            if port is not None:
                if not isinstance(port, int) or not (1 <= port <= 65535):
                    errors.append(
                        f"{prefix}: {port_field} must be an integer between 1 and 65535"
                    )
        ...
        if health_check is not None:
            health_errors = self._validate_health_check(
                health_check, f"{prefix}.health_check"
            )
        ...
        if restart_policy is not None:
            restart_errors = self._validate_restart_policy(
                restart_policy, f"{prefix}.restart_policy"
            )
    ...
    def _validate_health_check(
        self, health_check: dict[str, Any], prefix: str
    ) -> list[str]:
        ...
        for field, (min_val, max_val) in numeric_fields.items():
            ...
            if value is not None:
                if not isinstance(value, int | float) or not (
                    ...
                    errors.append(
                        f"{prefix}.{field}: must be a number between {min_val} and {max_val}"
                    )

        return errors

    def _validate_restart_policy(
        self, restart_policy: dict[str, Any], prefix: str
    ) -> list[str]:
    ...
    def _validate_defaults(self, defaults: dict[str, Any]) -> list[str]:
        ...
        if cluster_health is not None:
            cluster_errors = self._validate_cluster_health(
                cluster_health, "defaults.cluster_health"
            )
            errors.extend(cluster_errors)

        return errors

    def _validate_cluster_health(
        self, cluster_health: dict[str, Any], prefix: str
    ) -> list[str]:
            ...
            if not isinstance(commands, dict):
                ...
                for cmd_name, cmd_config in commands.items():
                    if cmd_name not in valid_commands:
                        errors.append(
                            f"{prefix}.commands.{cmd_name}: unknown command (valid: {valid_commands})"
                        )
                        ...
                        if isinstance(cmd_config, bool):
                            ...
                            if cmd_enabled is not None and not isinstance(
                                cmd_enabled, bool
                            ):
                                errors.append(
                                    f"{prefix}.commands.{cmd_name}.enabled: must be a boolean"
                                )
                        else:
                            errors.append(
                                f"{prefix}.commands.{cmd_name}: must be a boolean or dictionary"
                            )
    ...
    def _substitute_environment_variables(self, content: str) -> str:
        ...
        if substituted != content:
            ...
            logger.debug("Substituted environment variables", count=substitution_count)
    ...
    async def substitute_environment_variables(
    ...
    async def add_service_config(self, service: dict[str, Any]) -> None:
        ...
        try:
            ...
            if validation_errors:
                raise ConfigurationError(
                    f"Invalid service configuration: {'; '.join(validation_errors)}"
                )
            ...
            logger.info(
                "Successfully added service configuration",
            ...
            if backup_path and Path(backup_path).exists():
                logger.warning(
                    "Restoring configuration from backup due to error",
    ...
    async def remove_service_config(self, service_name: str) -> bool:
        ...
        try:
            ...
            logger.info(
                "Successfully removed service configuration",
                service_name=service_name,
    ...
    async def service_exists(self, service_name: str) -> bool:
        ...
        try:
            ...
            logger.debug(
                "Service existence check completed",
                service_name=service_name,
                exists=exists,
            )
            return exists
            ...
            logger.error(
                "Failed to check service existence",
                service_name=service_name,
                error=str(e),
            )
        ...
        try:
            ...
            for service in services:
                if service.get("name") == service_name:
                    logger.debug(
                        "Found service configuration", service_name=service_name
                    )
                    return service
    ...
    async def update_service_config(
        ...
        try:
            ...
            logger.info(
                "Successfully updated service configuration", service_name=service_name
            )
            return True
