...
class TaskManager:
    ...
    def register_task(
        ...
        *,
        group: str | None = None,
        cleanup_callback: Callable | None = None,
        resource_tags: set[str] | None = None,
        priority: int = 0,
    ) -> asyncio.Task:
    ...
    def _on_task_completed(self, name: str, task: asyncio.Task) -> None:
        ...
        if task.cancelled():
            ...
            logger.warning("Task failed", name=name, error=str(task.exception()))
        else:
        ...
        if task_info.cleanup_callback:
            try:
                ...
                logger.exception(
                    "Task cleanup callback failed", name=name, error=str(e)
                )
    ...
    def get_tasks_by_group(self, group: str) -> list[TaskInfo]:
        ...
        return [
            self._tasks[name]
            for name in self._task_groups[group]
            if name in self._tasks
        ]
    ...
    def get_running_tasks(self) -> list[TaskInfo]:
        ...
        return [
            info for info in self._tasks.values() if info.state == TaskState.RUNNING
        ]

    def get_tasks_by_resource_tag(self, tag: str) -> list[TaskInfo]:
        """Get all tasks with a specific resource tag."""
        return [info for info in self._tasks.values() if tag in info.resource_tags]

    async def cancel_task(self, name: str, timeout: float | None = None) -> bool:
        ...
        if task_info.state not in (TaskState.RUNNING, TaskState.PENDING):
            logger.debug(
                "Task not in cancellable state", name=name, state=task_info.state.value
            )
            return True
        ...
        try:
            await asyncio.wait_for(
                asyncio.shield(task_info.task), timeout=cancel_timeout
            )
            ...
            logger.warning(
                "Task raised exception during cancellation", name=name, error=str(e)
            )

        return task_info.task.cancelled() or task_info.task.done()

    async def cancel_group(self, group: str, timeout: float | None = None) -> int:
        ...
        cancel_tasks = [
            self.cancel_task(task_info.name, timeout) for task_info in tasks
        ]
        ...
        logger.info(
            "Task group cancellation completed",
            group=group,
    ...
    async def cancel_all_tasks(self, timeout: float | None = None) -> int:
        ...
        logger.info(
            "All tasks cancellation completed",
    ...
    async def wait_for_completion(self, timeout: float | None = None) -> bool:
        ...
        try:
            await asyncio.wait_for(
                asyncio.gather(*task_objects, return_exceptions=True), timeout=timeout
            )
            return True
        except TimeoutError:
            logger.warning(
