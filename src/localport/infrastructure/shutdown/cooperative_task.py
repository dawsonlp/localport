...
class CooperativeTask(GracefulShutdownMixin):
    ...
    def __init__(self, name: str, check_interval: float = 5.0):
        ...
        self._running = False
        self._iterations = 0
        self._last_iteration_time: float | None = None

        logger.debug(
            "CooperativeTask initialized", task_name=name, check_interval=check_interval
        )
    ...
    async def start(self) -> None:
        ...
        self._task_handle = asyncio.create_task(
            self._run_loop(), name=f"cooperative_task_{self._task_name}"
        )
    ...
    async def stop(self) -> None:
        ...
        if self._task_handle and not self._task_handle.done():
            try:
                ...
                logger.warning(
                    "Task did not stop gracefully, cancelling",
    ...
    async def _run_loop(self) -> None:
        ...
        try:
            while not self.is_shutdown_requested():
                ...
                try:
                    ...
                    if not should_continue:
                        logger.error(
                            "Task stopping due to iteration error",
                            task_name=self._task_name,
                        )
                        break
                ...
                if remaining_wait > 0:
                    ...
                    shutdown_requested = await self.wait_for_shutdown_or_timeout(
                        remaining_wait
                    )
                    if shutdown_requested:
                        logger.debug(
                            "Shutdown requested during wait", task_name=self._task_name
                        )
                        break
            ...
            logger.exception(
                "Task loop failed", task_name=self._task_name, error=str(e)
            )
            raise
        finally:
            logger.debug(
    ...
    async def _handle_iteration_error(self, error: Exception) -> bool:
        ...
        logger.warning(
            "Task iteration error, continuing",
            task_name=self._task_name,
            error=str(error),
        )
        return True
