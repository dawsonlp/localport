...
class ShutdownCoordinator:
    ...
    def __init__(
        ...
        phase_configs: dict[ShutdownPhase, PhaseConfig] | None = None,
    ):
        ...
        logger.debug(
            "ShutdownCoordinator initialized", phases=list(self._phase_configs.keys())
        )
    ...
    def register_phase_callback(self, phase: ShutdownPhase, callback: Callable) -> None:
        ...
        logger.debug(
            "Phase callback registered", phase=phase.value, callback=callback.__name__
        )
    ...
    def update_phase_timeout(self, phase: ShutdownPhase, timeout: float) -> None:
        ...
        if phase in self._phase_configs:
            ...
            logger.debug("Phase timeout updated", phase=phase.value, timeout=timeout)
    ...
    async def _execute_phase(self, phase: ShutdownPhase) -> bool:
        ...
        try:
            ...
            logger.warning(
                "Shutdown phase timed out", phase=phase.value, timeout=config.timeout
            )
            success = False
        except Exception as e:
            logger.exception("Shutdown phase failed", phase=phase.value, error=str(e))
            success = False
        finally:
        ...
        if success:
            logger.info(
                "Shutdown phase completed", phase=phase.value, duration=phase_duration
            )
        else:
            logger.error(
                "Shutdown phase failed", phase=phase.value, duration=phase_duration
            )
    ...
    async def _execute_phase_callbacks(
        self, phase: ShutdownPhase, timeout: float
    ) -> None:
        ...
        logger.debug(
            "Executing phase callbacks", phase=phase.value, count=len(callbacks)
        )
    ...
    def get_current_phase(self) -> ShutdownPhase | None:
    ...
    async def emergency_shutdown(self, timeout: float = 5.0) -> bool:
        ...
        try:
            ...
            duration = time.time() - start_time
            logger.exception(
                "Emergency shutdown failed", duration=duration, error=str(e)
            )
            return False
