...
class AsyncSignalHandler:
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop | None = None):
        ...
        logger.debug(
            "AsyncSignalHandler initialized",
    ...
    def setup_signal_handlers(self) -> None:
        ...
        """
        logger.info("Setting up signal handlers", signals=list(self._signal_map.keys()))

        for sig_num, sig_type in self._signal_map.items():
            try:
                # Store original handler for cleanup
                self._original_handlers[sig_num] = signal.signal(
                    sig_num, signal.SIG_DFL
                )
                ...
                if sys.platform == "win32":
                    ...
                    self._loop.add_signal_handler(
                        sig_num, self._async_signal_handler, sig_num, sig_type
                    )

                self._signal_handlers[sig_num] = sig_type
                logger.debug(
                    "Signal handler installed", signal=sig_num, type=sig_type.value
                )

            except (OSError, ValueError) as e:
                logger.warning(
                    "Failed to install signal handler", signal=sig_num, error=str(e)
                )
    ...
    def _signal_handler(self, signum: int, frame) -> None:
        ...
        if sig_type:
            self._loop.call_soon_threadsafe(self._handle_signal_async, signum, sig_type)
    ...
    async def _handle_shutdown_signal(self, signum: int) -> None:
        ...
        with self._signal_lock:
            if self._shutdown_initiated:
                logger.debug(
                    "Shutdown already initiated, ignoring signal", signal=signum
                )
                return
    ...
    def cleanup_signal_handlers(self) -> None:
        ...
        for sig_num in list(self._signal_handlers.keys()):
            try:
                ...
                logger.warning(
                    "Failed to cleanup signal handler", signal=sig_num, error=str(e)
                )
