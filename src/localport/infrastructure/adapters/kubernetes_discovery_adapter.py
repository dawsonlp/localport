...
from ...domain.exceptions import KubernetesResourceNotFoundError
from ...domain.repositories.discovery_repository import KubernetesDiscoveryRepository
from ...domain.value_objects.discovery import DiscoveredPort, KubernetesResource
...
class KubernetesDiscoveryAdapter(KubernetesDiscoveryRepository):
    ...
    async def find_resource(
        ...
        for resource_type in resource_types:
            try:
                resource = await self._find_resource_by_type(
                    name, resource_type, namespace
                )
                if resource:
                    logger.debug(
                        "Found resource",
    ...
    async def get_available_ports(
        self, resource_name: str, namespace: str
    ) -> list[DiscoveredPort]:
        ...
        """
        logger.debug(
            "Getting available ports", resource_name=resource_name, namespace=namespace
        )
        ...
        if not resource:
            raise KubernetesResourceNotFoundError(
                f"Resource '{resource_name}' not found in namespace '{namespace}'"
            )
        ...
        if resource.resource_type == "service":
            ...
            logger.warning(
                "Unknown resource type for port discovery",
    ...
    async def get_current_namespace(self) -> str:
        ...
        try:
            ...
            if returncode != 0:
                logger.warning(
                    "Failed to get current namespace, using default", error=stderr
                )
                return "default"
    ...
    async def search_all_namespaces(
        ...
        for resource_type in resource_types:
            try:
                namespace_matches = await self._search_resource_all_namespaces(
                ...
                logger.warning(
                    "Error searching all namespaces",
                    resource_type=resource_type,
                    error=str(e),
                )
                continue
        ...
        logger.debug(
            "Search completed", resource_name=resource_name, matches_found=len(matches)
        )
        return matches
    ...
    async def _find_resource_by_type(
        ...
        try:
            ...
            cmd = (
                ["kubectl", "get", resource_type, name, "--output=json"]
                + namespace_args
                + context_args
            )
            ...
            if returncode != 0:
                if (
                    ...
                    logger.warning(
                        "kubectl command failed", cmd=" ".join(cmd), error=stderr
                    )
                    return None
    ...
    async def _search_resource_all_namespaces(
        ...
        try:
            ...
            if returncode != 0:
                logger.warning(
                    "Failed to search all namespaces",
                    resource_type=resource_type,
                    error=stderr,
                )
                return []
    ...
    async def _parse_resource_data(
        ...
        try:
            ...
            available_ports = await self._extract_ports_from_resource_data(
    ...
    async def _extract_ports_from_resource_data(
        ...
        try:
            ...
            logger.warning(
                "Error extracting ports from resource data",
                resource_type=resource_type,
                error=str(e),
            )
    ...
    async def _get_service_ports(
        ...
        try:
            ...
            if returncode != 0:
                raise KubernetesResourceNotFoundError(
                    f"Service '{service_name}' not found in namespace '{namespace}': {stderr}"
                )
    ...
    async def _get_pod_ports(
        ...
        try:
            ...
            if returncode != 0:
                raise KubernetesResourceNotFoundError(
                    f"Pod '{pod_name}' not found in namespace '{namespace}': {stderr}"
                )
    ...
    async def _get_deployment_ports(
        ...
        try:
            ...
            if returncode != 0:
                raise KubernetesResourceNotFoundError(
                    f"Deployment '{deployment_name}' not found in namespace '{namespace}': {stderr}"
                )

            deployment_data = json.loads(stdout)
            return await self._extract_ports_from_resource_data(
