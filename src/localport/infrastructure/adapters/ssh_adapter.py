...
class SSHAdapter(PortForwardingAdapter):
    ...
    async def start_port_forward_with_logging(
        ...
        password = connection_info.config.get(
            "password"
        )  # Direct access for password since no method exists
        ...
        if key_file:
            ...
            if not key_path.exists():
                ...
                raise SSHKeyNotFoundError(
                    key_path=str(key_path), service_name=service_name
                )
        ...
        logger.info(
            "Starting SSH tunnel with logging",
        ...
        try:
            ...
            logger.info(
                "SSH subprocess created with logging",
                pid=process.pid,
                service_id=service_id,
            )
            ...
            logger.info(
                "SSH tunnel started successfully with logging",
                pid=process.pid,
                service_id=service_id,
                local_port=local_port,
                remote_port=remote_port,
            ...
            if "log_file_handle" in locals():
                log_file_handle.close()
            logger.error(
                "Failed to start SSH tunnel with logging",
                error=str(e),
                service_id=service_id,
                local_port=local_port,
                remote_port=remote_port,
    ...
    async def start_port_forward(
        ...
        cmd = [
            ...
            "UserKnownHostsFile=/dev/null",  # Don't save host keys
            "-o",
            "LogLevel=ERROR",  # Reduce SSH output
            "-o",
            "ServerAliveInterval=30",  # Keep connection alive
            "-o",
            "ServerAliveCountMax=3",  # Max missed keepalives
        ...
        if key_file:
            ...
            if not key_path.exists():
                ...
                raise SSHKeyNotFoundError(key_path=str(key_path))
            cmd.extend(["-i", str(key_path)])
    ...
    async def cleanup_all_processes(self) -> None:
        ...
        logger.info(
            "Cleaning up all SSH tunnel processes", count=len(self._active_processes)
        )
    ...
    async def test_ssh_connection(
        ...
        try:
            ...
            logger.error(
                "Error testing SSH connection", host=host, port=port, error=str(e)
            )
            return False
    ...
    async def validate_ssh_connectivity(
    ...
    async def check_prerequisites(self) -> bool:
        ...
        try:
            ...
            if not all_available:
                logger.warning(
                    "SSH adapter prerequisites not met", missing_tools=missing_tools
                )
                return False
            ...
            logger.error("Error checking SSH adapter prerequisites", error=str(e))
            return False
    ...
    async def validate_connection_info(
        ...
        try:
            ...
            if not host or not host.strip():
                errors.append(
                    "SSH host cannot be empty. Provide a hostname like 'example.com' or IP address like '192.168.1.100'"
                )
        except ValueError:
            errors.append(
                "SSH connection requires 'host' field. Example: host: 'example.com' or host: '192.168.1.100'"
            )
        ...
        try:
            ...
            if not 1 <= port <= 65535:
                errors.append(
                    f"SSH port {port} must be between 1 and 65535 (default SSH port is 22)"
                )
        except (ValueError, TypeError):
            errors.append(
                "SSH port must be a valid integer. Example: port: 22 or port: 2222"
            )
        ...
        if key_file:
            ...
            if not key_path.exists():
                ...
                errors.append(
                    f"SSH key file not found: {safe_path}. Check path or generate key: ssh-keygen -t rsa"
                )
        ...
        if not has_key and not has_password:
            errors.append(
                "SSH connection requires either 'key_file' or 'password' for authentication"
            )
        ...
        if has_password and not has_key:
            ...
            try:
                ...
                if not shutil.which("sshpass"):
                    errors.append(
                        "Password authentication requires 'sshpass' to be installed. Install with: brew install sshpass (macOS) or apt-get install sshpass (Ubuntu)"
                    )
            except Exception:
                pass
