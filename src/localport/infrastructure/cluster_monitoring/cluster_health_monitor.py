...
class ClusterHealthMonitor:
    ...
    def __init__(self, context: str, config: ClusterMonitorConfig):
        ...
        self.kubectl_client = KubectlClient(
            timeout=config.timeout, retry_attempts=config.retry_attempts
        )

        # State tracking
        self._current_health: ClusterHealth | None = None
        ...
        self._cached_cluster_info: ClusterInfo | None = None
    ...
    async def start_monitoring(self) -> None:
        ...
        if self._is_running:
            raise RuntimeError(
                f"Monitoring already running for context '{self.context}'"
            )
    ...
    async def get_current_health(self) -> ClusterHealth | None:
        """
    ...
    async def refresh_health(self) -> ClusterHealth | None:
        """
    ...
    async def get_cluster_info(self) -> ClusterInfo | None:
        """
    ...
    async def get_resource_status(
        self, namespace: str, resource_name: str, resource_type: str
    ) -> ResourceStatus | None:
        """
    ...
    async def _monitoring_loop(self) -> None:
        ...
        logger.debug(
            f"Starting monitoring loop for {self.context} (interval: {self.config.interval}s)"
        )
        ...
        try:
            ...
            if self.config.enable_cluster_info:
                ...
                if not cluster_info.is_reachable:
                    ...
                    logger.warning(
                        f"Cluster {self.context} is not reachable: {cluster_info.error_message}"
                    )
                    return
            ...
            if self.config.enable_node_status:
                try:
                    ...
                    logger.warning(
                        f"Failed to get node statuses for {self.context}: {e}"
                    )
            ...
            if self.config.enable_pod_status:
                try:
                    ...
                    logger.warning(
                        f"Failed to get pod statuses for {self.context}: {e}"
                    )
            ...
            if self.config.enable_events_on_failure and (
                ...
                try:
                    events = await self.kubectl_client.get_cluster_events(
                        self.context, limit=50
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to get cluster events for {self.context}: {e}"
                    )
            ...
            logger.debug(
                f"Health check completed for {self.context}: {self._current_health.status_summary}"
            )
    ...
    def get_monitoring_stats(self) -> dict[str, Any]:
        ...
        return {
            ...
            "current_status": (
                self._current_health.status.value if self._current_health else "Unknown"
            ),
            "config": {
