...
class KubectlClient:
    ...
    async def get_cluster_info(self, context: str) -> ClusterInfo:
        ...
        try:
            ...
            try:
                ...
                version_stdout, _, version_returncode = await self._execute_command(
                    version_cmd
                )
                ...
                if version_returncode == 0:
                    try:
                        ...
                        if server_version:
                            cluster_version = server_version.get(
                                "gitVersion", ""
                            ).replace("v", "")
    ...
    async def get_node_statuses(self, context: str) -> list[ResourceStatus]:
        ...
        """
        try:
            cmd = ["kubectl", "get", "nodes", "--output=json", "--context", context]
    ...
    async def _execute_command(self, cmd: list[str]) -> tuple[str, str, int]:
        ...
        for attempt in range(self.retry_attempts + 1):
            try:
                logger.debug(
                    f"Executing kubectl command (attempt {attempt + 1}): {' '.join(cmd)}"
                )
    ...
    def _parse_pod_status(
        self, pod_data: dict[str, Any], context: str
    ) -> ResourceStatus | None:
        ...
        try:
            ...
            for cond_data in status.get("conditions", []):
                condition = ResourceCondition(
                    ...
                    last_transition_time=self._parse_timestamp(
                        cond_data.get("lastTransitionTime")
                    ),
                )
    ...
    def _parse_node_status(
        self, node_data: dict[str, Any], context: str
    ) -> ResourceStatus | None:
    ...
    def _parse_cluster_event(
        self, event_data: dict[str, Any], context: str
    ) -> ClusterEvent | None:
