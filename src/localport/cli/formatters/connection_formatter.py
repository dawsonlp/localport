...
class ConnectionTableFormatter:
    ...
    def format_connections_list(self, response: ListConnectionsResponse) -> None:
        ...
        if not response.services:
            ...
            self.console.print(
                "Use 'localport config add' to add your first connection."
            )
            return
    ...
    def _format_summary(self, response: ListConnectionsResponse) -> None:
        ...
        summary_table = Table(
            title="📊 Summary", show_header=False, title_style="bold blue", box=None
        )
        ...
        self.console.print(
            "[dim]  • localport status        - Check running services[/dim]"
        )

    def format_add_success(
        self, service_name: str, technology: str, local_port: int, target_info: str
    ) -> None:
        ...
        self.console.print("\n[dim]💡 Next steps:[/dim]")
        self.console.print(
            f"[dim]  • localport start {service_name}  - Start the service[/dim]"
        )
        self.console.print(
            "[dim]  • localport status             - Check service status[/dim]"
        )

    def format_remove_success(
        self, service_name: str, was_running: bool = False
    ) -> None:
        ...
        if was_running:
            self.console.print(
                "[yellow]⚠️  Note: Service was running and has been stopped.[/yellow]"
            )
