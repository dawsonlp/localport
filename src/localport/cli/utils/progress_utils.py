...
class EnhancedProgress:
    ...
    def create_spinner_progress(self, show_elapsed: bool = True) -> Progress:
        ...
        return Progress(*columns, console=self.console, transient=False, expand=False)
    ...
    async def run_with_spinner(
        ...
        with self.create_spinner_progress(show_elapsed) as progress:
            ...
            try:
                ...
                progress.update(
                    task, description=f"✅ {success_message or description}"
                )
    ...
    async def run_with_steps(
        self, steps: list[tuple[str, Callable]], overall_description: str = "Processing"
    ) -> list[Any]:
