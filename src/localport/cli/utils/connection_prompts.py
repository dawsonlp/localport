...
class ConnectionPrompts:
    ...
    @staticmethod
    async def prompt_for_local_port(suggested: int | None = None) -> int:
        ...
        if suggested:
            ...
            use_suggested = Confirm.ask(
                f"Use suggested port {suggested}?", default=True
            )
        ...
        while True:
            try:
                port = IntPrompt.ask(
                    "Enter local port", default=suggested or 8080, show_default=True
                )
    ...
    async def prompt_for_namespace_selection(namespaces: list[str]) -> str:
        ...
        while True:
            try:
                ...
                console.print(
                    f"[red]Error: Please enter a number between 1 and {len(namespaces)}[/red]"
                )
                continue
    ...
    async def prompt_for_port_selection(ports: list[DiscoveredPort]) -> DiscoveredPort:
        ...
        if len(ports) == 1:
            console.print(
                f"[green]Auto-selecting the only available port: {ports[0].port}[/green]"
            )
            return ports[0]
        ...
        while True:
            try:
                ...
                console.print(
                    f"[red]Error: Please enter a number between 1 and {len(ports)}[/red]"
                )
                continue
    ...
    @staticmethod
    async def prompt_for_ssh_user() -> str | None:
        ...
        """
        user = Prompt.ask(
            "Enter SSH username (optional, press Enter to skip)", default=""
        ).strip()
    ...
    async def prompt_for_ssh_remote_port(default: int = 22) -> int:
        ...
        while True:
            try:
                port = IntPrompt.ask(
                    "Enter SSH port", default=default, show_default=True
                )
    ...
    @staticmethod
    async def prompt_for_ssh_key_file() -> str | None:
        ...
        """
        key_file = Prompt.ask(
            "Enter path to SSH key file (optional, press Enter to skip)", default=""
        ).strip()

        return key_file if key_file else None

    @staticmethod
    async def prompt_for_service_name(suggested: str | None = None) -> str:
        ...
        if suggested:
            ...
            use_suggested = Confirm.ask(
                f"Use suggested name '{suggested}'?", default=True
            )
        ...
        while True:
            name = Prompt.ask("Enter service name", default=suggested or "").strip()
    ...
    async def confirm_service_removal(
        service_name: str, is_running: bool = False
    ) -> bool:
        ...
        if is_running:
            console.print(
                "[bold red]⚠️  WARNING: This service is currently running![/bold red]"
            )
            console.print(
                "You may want to stop the service first using: localport stop {service_name}"
            )
    ...
    @staticmethod
    async def prompt_for_kubectl_namespace(current_namespace: str) -> str | None:
        ...
        console.print(
            f"Current namespace: [bold green]{current_namespace}[/bold green]"
        )

        use_current = Confirm.ask(
            f"Use current namespace '{current_namespace}'?", default=True
        )
        ...
        namespace = Prompt.ask("Enter namespace", default="").strip()

        return namespace if namespace else None

    @staticmethod
    def show_discovery_progress(resource_name: str, namespace: str | None = None):
        ...
        if namespace:
            console.print(
                f"[dim]🔍 Discovering resource '{resource_name}' in namespace '{namespace}'...[/dim]"
            )
        else:
            console.print(
                f"[dim]🔍 Discovering resource '{resource_name}' across all namespaces...[/dim]"
            )
    ...
    def show_validation_progress(service_name: str):
        ...
        """
        console.print(
            f"[dim]✓ Validating service configuration for '{service_name}'...[/dim]"
        )
