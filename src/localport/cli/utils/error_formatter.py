...
class ErrorFormatter:
    ...
    def format_error(
        self, error: Exception, verbosity: VerbosityLevel = VerbosityLevel.NORMAL
    ) -> Panel:
        """
    ...
    def _format_localport_error(
        self, error: LocalPortError, verbosity: VerbosityLevel
    ) -> Panel:
    ...
    def _build_verbose_content(self, error: LocalPortError, icon: str) -> Text:
        ...
        if error.context:
            ...
            for key, value in error.context.items():
                if (
                    value is not None and key != "key_path"
                ):  # Hide full paths in verbose
    ...
    def _format_generic_error(
        self, error: Exception, verbosity: VerbosityLevel
    ) -> Panel:
    ...
    def format_multiple_errors(
        self, errors: list[Exception], verbosity: VerbosityLevel = VerbosityLevel.NORMAL
    ) -> Panel:
        """
        ...
        for i, error in enumerate(errors, 1):
            ...
            if isinstance(error, LocalPortError):
                ...
                if verbosity != VerbosityLevel.NORMAL and error.suggestions:
                    for suggestion in error.suggestions[
                        :2
                    ]:  # Limit suggestions in multi-error view
                        content.append(f"   💡 {suggestion}\n", style="cyan dim")
            else:
                content.append(f"⚠️ {str(error)}\n", style="red")
