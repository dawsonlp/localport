...
from ...infrastructure.adapters.kubernetes_discovery_adapter import (
    ...
    try:
        ...
        if not active_config or not active_config.exists:
            if output_format == OutputFormat.JSON:
                ...
                search_paths_text = (
                    await ConfigPathManager.format_search_paths_with_status()
                )
                console.print(
                    create_info_panel(
        ...
        if format.lower() == "json":
            ...
            raise ValueError(
                f"Unsupported export format: {format}. Supported formats: yaml, json"
            )
...
async def validate_config_command(
    config_file: str | None = None, output_format: OutputFormat = OutputFormat.TABLE
) -> None:
    ...
    try:
        ...
        if output_format == OutputFormat.JSON:
            ...
            if validation_results:
                table = Table(
                    title=f"Configuration Validation: {Path(config_path).name}"
                )
...
def export_config_sync(
    ...
    format: str = typer.Option(
        "yaml", "--format", "-f", help="Export format (yaml, json)"
    ),
    include_defaults: bool = typer.Option(
        True, "--include-defaults/--no-defaults", help="Include default settings"
    ),
    include_disabled: bool = typer.Option(
        False, "--include-disabled", help="Include disabled services"
    ),
    services: list[str] | None = typer.Option(
        None, "--service", "-s", help="Export specific services only"
    ),
    tags: list[str] | None = typer.Option(
        None, "--tag", "-t", help="Export services with specific tags only"
    ),
) -> None:
    """Export LocalPort configuration to different formats.
...
def validate_config_sync(
    ...
    config_file: str | None = typer.Option(
        None, "--config", "-c", help="Configuration file to validate"
    ),
) -> None:
    """Validate LocalPort configuration file.
    ...
    try:
        ...
        # Handle SSH connection
            ...
            raise ValidationError(
                f"Unsupported technology: {technology}. Use 'kubectl' or 'ssh'"
            )
...
async def _handle_kubectl_connection(
    ...
    if namespace is None:
        namespace = await ConnectionPrompts.prompt_for_kubectl_namespace(
            current_namespace
        )
    ...
    request = AddConnectionRequest(
        ...
        connection_params={"resource_name": resource_name, "namespace": namespace},
        options={"local_port": local_port, "remote_port": remote_port},
    )
    ...
    # Format output
    formatter = ConnectionFormatterFactory.create_formatter(
        output_format.value, console
    )
...
async def _handle_ssh_connection(
    ...
    if not local_port:
        local_port = await ConnectionPrompts.prompt_for_local_port(
            suggested=remote_port
        )
    ...
    # Create connection request
    connection_params = {"host": ssh_host, "port": ssh_port}
    if ssh_user:
...
async def remove_connection_command(
    ...
    try:
        # Initialize repositories and services
        config_repo = YamlConfigRepository()
        remove_use_case = RemoveConnectionUseCase(
            config_repo, None
        )  # Service repo not needed for config removal

        # Check if service exists
        if not await config_repo.service_exists(service_name):
            console.print(
                f"[red]Error: Service '{service_name}' not found in configuration.[/red]"
            )
            raise typer.Exit(1)

        # Confirm removal if not forced
        if not force:
            confirmed = await ConnectionPrompts.confirm_service_removal(
                service_name, False
...
async def list_connections_command(
    output_format: OutputFormat = OutputFormat.TABLE, verbosity: int = 0
) -> None:
...
def add_connection_sync(
    ctx: typer.Context,
    service_name: str | None = typer.Option(None, "--name", "-n", help="Service name"),
    technology: str | None = typer.Option(
        None, "--technology", "-t", help="Technology (kubectl/ssh)"
    ),
    resource_name: str | None = typer.Option(
        None, "--resource", "-r", help="Kubernetes resource name"
    ),
    namespace: str | None = typer.Option(
        None, "--namespace", help="Kubernetes namespace"
    ),
    local_port: int | None = typer.Option(
        None, "--local-port", "-l", help="Local port"
    ),
    remote_port: int | None = typer.Option(None, "--remote-port", help="Remote port"),
    ssh_host: str | None = typer.Option(None, "--host", help="SSH hostname"),
    ssh_user: str | None = typer.Option(None, "--user", "-u", help="SSH username"),
    ssh_key: str | None = typer.Option(None, "--key", "-k", help="SSH key file"),
...
def remove_connection_sync(
    ...
    asyncio.run(
        remove_connection_command(
            service_name, force, cli_ctx.output_format, cli_ctx.verbosity_level
        )
    )


def list_connections_sync(ctx: typer.Context) -> None:
    """List all configured connections.
    ...
    asyncio.run(
        list_connections_command(cli_ctx.output_format, cli_ctx.verbosity_level)
    )
