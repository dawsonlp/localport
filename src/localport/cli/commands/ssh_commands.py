...
async def test_ssh_connectivity_command(
    ...
    try:
        ...
        if success:
            ...
            table.add_row(
                "Password Auth", "yes" if connection_info.get("password") else "no"
            )
...
async def validate_ssh_config_command(
    config_file: str | None = None, service_name: str | None = None
) -> None:
    ...
    try:
        ...
        for service in ssh_services:
            ...
            validation_results.append(
                {"service": service, "errors": errors, "valid": len(errors) == 0}
            )
...
def test_ssh_connectivity_sync(
    ...
    service_name: str | None = typer.Argument(
        None, help="Service name to test (from config)"
    ),
    ...
    key_file: str | None = typer.Option(
        None, "--key-file", "-k", help="SSH private key file"
    ),
    ...
    asyncio.run(
        test_ssh_connectivity_command(
            service_name, host, user, port, key_file, effective_config
        )
    )
...
def validate_ssh_config_sync(
    ...
    service_name: str | None = typer.Option(
        None, "--service", "-s", help="Specific service to validate"
    ),
) -> None:
    """Validate SSH configuration in a config file."""
    cli_ctx = get_cli_context(ctx)
